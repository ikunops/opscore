...
3. 读投影中**仅 `ReadAll` 填充 `MinSeq/MaxSeq`**（file_transition_store.go:770-771；Load/ReadRecent 留零值，`TransitionPageResult` 无此字段，alerting.go:173-189）；且**不暴露逐记录 seq**；ring 两级有界（256 + file 10000，P30）⇒ 合法逐出只裁最旧，**不存在上界合法逐出**。
4. `acceptance / record_digest` 全仓**零命中**（`integrity` 一词仅出现在无关注释）——输入面零完整性证据。
...
Append **持久化成功后**（durable Sync 完成 file_transition_store.go:454-458；水位推进 :462-470，P32-I15——钩子在其后），store 在**同一临界区内**计算 `record_digest = sha256(durable 行字节)` 并追加接受条目：
...
### A2 — canonical 字节承诺（零第二序列化器）

`record_digest = sha256(json.Marshal(newPersisted(seq, t)))` —— 与写入路径**同一构造函数同一字段序**的 canonical 字节（对账时由解析后 struct 重序列化重建，机制见 ADR-066 §5）。理由：①不引入第二套序列化器（纪律）；②任何字段值变化即 modified；语义等价的 JSON 重写（换键序/加空白）**正确地**判 intact——记录内容未变；③规避原始行字节路线的假阳性，re-marshal 漂移由 newPersisted 唯一构造点 + T257 反向守卫钉死。
...
| 账本条目 record_seq **> MaxSeq** | **`record_missing_in_span`（可断言删除）**——上界不存在合法逐出（evictLocked 只裁最旧），删最新/回滚文件在此暴露 |
| 账本条目 record_seq < MinSeq | `outside_span` ⇒ **不断言**（ring 合法逐出与删除不可区分，R40-1 边界纪律） |
| store 不可读 / `Corrupt` / `LoadErr` | **`input_unverifiable` + `input_error`**（快照不可信 ⇒ 绝不 input_ok 假绿） |
...
- 接受条目在锚定启用时进 P40 通道：`kind=acceptance`，**独立文件** `acceptance-anchor.jsonl`（第五锚定流，I5 自指教训：**该流自身 compaction 不被观察**）。
...
2. **删最老掩饰（下界缝隙）**：MinSeq/MaxSeq 均为读时派生（快照内现存记录的界，非可篡改元数据）——抬高 MinSeq 的唯一途径是**删除最老记录**，其效果与合法 ring 逐出不可区分 ⇒ 该区间 outside_span 不断言（R40-1 家族代价）。**上界无此缝隙**：不存在合法上界逐出 ⇒ 删最新/回滚文件被 missing_in_span 捕获（T254 钉住）。store 文件被删重建 ⇒ seq 从 1 重分配 ⇒ 对旧账本大面积 `record_modified`（响亮但诊断指向失真，非静默——运维已知态）。
...
| # | 断言 |
|---|---|
| T237 | 未启用 ⇒ 不建账本、输出逐字节不变（Append 热路径仅一个 nil 判断） |
| T238 | Append ⇒ 接受条目落账本（record_seq 对齐、digest = durable 行字节哈希、KAK 签名验真） |
| T239 | **核心红例（non-vacuousness）**：篡改 durable 记录字节 ⇒ P35~P44 全绿 ⇒ 仅 P45 观测 `record_modified` |
| T240 | 删记录（span 内）⇒ `record_missing_in_span` |
| T241 | seq < MinSeq ⇒ `outside_span` 不断言（与 T239 同 fixture 判别） |
| T242 | record-first 崩溃窗 ⇒ `record_unaccepted`（响亮） |
| T243 | 账本坏行 ⇒ fail-closed 拒追加 + `input_error`，文件字节不动 |
| T244 | 同 entry_seq 第二行（含同 payload）⇒ conflict（事实非状态） |
| T245 | 锚定第五流 + acceptance 流自身 compaction 不被观察 + acceptance_compaction 记入销毁账本（无环） |
| T246 | 跨维零干扰：P35~P44 全部取值零变化 |
| T247 | 冻结面零 diff（protection 核心仅 alerting.go Seqs additive） |
| T248 | 并发 Append 全部被记账，entry_seq 无跳空（P43 临界区教训） |
| T249 | 锁内无 witness I/O（派发收集后置） |
| T250 | 删最老 ⇒ 对应区间 outside_span 不断言（诚实红例：不可断言，绝不误报 missing） |
| T251 | 账本窗口 + coverage_floor 强制输出；段不连续 ⇒ `window_discontinuous` |
| T252 | coverage floor 之前的记录 ⇒ outside_coverage（启用前历史，绝不 unaccepted） |
...
不降级任何既有判据；input_integrity 作为与 anchor（P40）/lifecycle（P41）/verification（P42）/destruction（P43）/verifier（P44）并列的独立正交维度新增。

## 7. 提请评审确认的取舍（已裁决，可反驳）

- **Q1** digest 基于 canonical 字节（写入与对账同一构造函数重序列化）而非独立第二套 canonical —— 采纳（A2 理由；原始行字节路线因空白/键序假阳性被否）。
